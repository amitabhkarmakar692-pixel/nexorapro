...
export default function SellerSignupPage() {
  ...
  async function handleSubmit() {
    if (!supabase) {
      setError("Supabase is not configured. Please define NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in your settings.");
      return;
    }
    if (!transactionId) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4 py-10">
      <div className="bg-white rounded-2xl border border-slate-200 w-full max-w-lg p-8">
        ...
        {!isConfigured && (
          <div className="bg-amber-50 border border-amber-200 text-amber-800 text-xs px-4 py-3 rounded-xl mb-5 text-left">
            <p className="font-bold mb-1">⚠️ Supabase is not configured</p>
            <p className="leading-relaxed">
              Please go to the <a href="/admin/settings" className="underline font-bold text-amber-900">Admin Settings</a> page to set up your Supabase database, or define <code className="bg-amber-100 px-1 rounded font-mono">NEXT_PUBLIC_SUPABASE_URL</code> and <code className="bg-amber-100 px-1 rounded font-mono">NEXT_PUBLIC_SUPABASE_ANON_KEY</code>.
            </p>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            <button
              onClick={async () => {
                try {
                  if (!supabase) {
                    throw new Error("Supabase is not configured. Please define NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in your settings.");
                  }
